...
class JobService:
    ...
    async def enqueue(self, kind: str, params: dict[str, Any]) -> Job:
        ...
        job = await self._store.create(kind=kind, params=params)
    ...
    async def start_example(self, data: ExampleJobCreate) -> Job:
        """Start the example job."""
        return await self.enqueue("example", data.model_dump())
